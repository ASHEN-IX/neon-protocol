...
export default function GridScan({
  ...
  const pushScan = (timeSec) => {
    ...
    const packed = new Array(maxScans).fill(0);
    for (let i = 0; i < scans.length && i < maxScans; i += 1) {
      packed[i] = scans[i];
    }
    uniforms.uScanStarts.value = packed;
  ...
  useEffect(() => {
    ...
    const uniforms = {
      ...
      uLineJitter: { value: clamp01(lineJitter || 0) },
      uScanOpacity: { value: clamp01(scanOpacity) },
      uNoise: { value: Math.max(0, noiseIntensity) },
      uBloomOpacity: { value: Math.max(0, bloomIntensity) },
      uScanGlow: { value: Math.max(0.1, scanGlow) },
      uScanSoftness: { value: Math.max(0.1, scanSoftness) },
      uPhaseTaper: { value: clamp01(scanPhaseTaper) },
  ...
  useEffect(() => {
    ...
    if (material) {
      ...
      uniforms.uLineJitter.value = clamp01(lineJitter || 0);
      uniforms.uBloomOpacity.value = Math.max(0, bloomIntensity);
      uniforms.uNoise.value = Math.max(0, noiseIntensity);
      uniforms.uScanGlow.value = Math.max(0.1, scanGlow);
      uniforms.uScanOpacity.value = clamp01(scanOpacity);
      uniforms.uScanDirection.value =
        scanDirection === "backward" ? 1 : scanDirection === "pingpong" ? 2 : 0;
      uniforms.uScanSoftness.value = Math.max(0.1, scanSoftness);
      uniforms.uPhaseTaper.value = clamp01(scanPhaseTaper);
  ...
  useEffect(() => {
    ...
    const start = async () => {
      if (!enableWebcam || !modelsReady || !video) {
        return;
      }
        ...
        if (ts - lastDetect >= 33) {
          ...
          try {
            ...
            if (result && result.detection) {
              ...
              const tilt = Math.atan2(rightCenter.y - leftCenter.y, rightCenter.x - leftCenter.x);
...
function medianPush(buffer, value, maxLen) {
  buffer.push(value);
